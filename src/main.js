...
// 2. Mobile Zoom Prevention & Visual Viewport Keyboard Anchor
function initMobileViewportFixes() {
  ...
  // Mobile Virtual Keyboard Anchor Handler (Keeps Modals above keyboard without hiding)
...
// 5. Batch Processing Orchestrator
bus.on('batch:start', async ({ toolId, queue, options }) => {
  ...
  try {
    if (tool.batchExecute && typeof tool.executeBatch === 'function') {
      ...
      // Standard item-by-item parallel or sequential processing
      for (let i = 0; i < queue.length; i++) {
        const file = queue[i];
        const processed = state.get('processedFiles').find(p => p.index === i);
        if (processed && processed.status === 'done') continue;

        try {
          state.updateProgress(i, 15, 'processing');
          const result = await tool.execute(file, options, (progress) => {
